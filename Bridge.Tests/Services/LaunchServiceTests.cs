...
public class LaunchServiceTests : IDisposable
{
    ...
    public LaunchServiceTests()
    {
        ...
        _cheatsDirectory = Path.Combine(Path.GetTempPath(), $"bridge_test_launch_cheats_{Guid.NewGuid()}");
    ...
    // Bridge-managed cheat file (cheat_database_path pointing at the per-game folder, and the
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), $"bridge_test_retroarch_{Guid.NewGuid()}");
    ...
    // override file, but only when a Bridge-managed cheat file already exists for this exact game -
    ...
    // only the one apply_cheats_after_load line Bridge itself owns is ever removed. Unlike that
    ...
    public async Task LaunchAsync_CheatFileExistsButNoCorePath_DoesNotWriteOverrideFile()
    {
        ...
        // Bridge understands - even if a cheat file coincidentally existed at the expected path,
        // it must never be surfaced to an emulator Bridge didn't install.
        var game = MakeGame("nes");
        var cmdExePath = Path.Combine(Environment.SystemDirectory, "cmd.exe");
