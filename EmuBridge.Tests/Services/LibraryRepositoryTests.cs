...
public class LibraryRepositoryTests : IDisposable
{
    ...
    {
        _dbPath = Path.Combine(Path.GetTempPath(), $"emubridge_test_{Guid.NewGuid()}.db");
    ...
    // emubridge.db (like every existing user's) whose atari2600 row still only has the pre-fix
