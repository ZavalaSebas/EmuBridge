...
public class CheatServiceTests : IDisposable
{
    ...
    {
        _cheatsDirectory = Path.Combine(Path.GetTempPath(), $"emubridge_test_cheats_{Guid.NewGuid()}");
    }
    ...
    // EmuBridge-managed cheat file exists for this game), so it's never removed by this method.
    private const string TestCheatDirectory = @"C:\EmuBridge\Cheats\test-game-id";

    private static (string Dir, string ExePath) CreateFakeRetroArchInstall()
    {
        var dir = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), $"emubridge_test_retroarch_config_{Guid.NewGuid()}")).FullName;
    ...
    // this one game). Writing EmuBridge's own keys must never clobber that.
    [Fact]
    ...
    // the one line EmuBridge itself owns for that toggle, never the user's own saved override content,
    ...
    public async Task ApplyCheatLaunchOverridesAsync_RetroArchCfgHasAbsoluteConfigDirectory_UsesThatDirectoryDirectly()
    {
        ...
        var game = MakeGame("nes");
        var (dir, exePath) = CreateFakeRetroArchInstall();
        var customConfigDir = Path.Combine(Path.GetTempPath(), $"emubridge_test_custom_config_{Guid.NewGuid()}");
