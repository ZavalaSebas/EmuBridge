...
public class RomScannerServiceTests : IDisposable
{
    ...
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), $"emubridge_scan_test_{Guid.NewGuid()}");
    ...
    [Fact]
    public async Task AddScanFolderAsync_FolderDoesNotExist_ThrowsEmuBridgeException()
    {
        var folder = new ScanFolder { Id = Guid.NewGuid(), Path = Path.Combine(_tempRoot, "does-not-exist") };

        await Assert.ThrowsAsync<EmuBridgeException>(() => _scanner.AddScanFolderAsync(folder));
