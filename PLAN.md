...
- Extend `atari2600`'s recognized extensions to include `.bin` (the common headerless Atari 2600 dump format) — found as a real gap during interactive use (two real `.bin` ROMs went undetected, falling to the `unknown` fallback). Not fixed yet because `.bin` is ambiguous across other systems' disc images; needs a real decision on scope, not a reflexive one-line addition. See DEVELOPMENT.md → Known Limitations.
