...
- `KnownEmulators.json` catalog (embedded resource) — RetroArch 1.22.2 entry and all 15 of 15 platform cores independently verified: `nes`→FCEUmm, `snes`→Snes9x, `gb`/`gbc`→SameBoy, `gba`→mGBA, `n64`→Mupen64Plus-Next, `nds`→melonDS DS, `genesis`/`sms`/`gamegear`→Genesis Plus GX, `atari2600`→Stella, `atari7800`→ProSystem, `pcengine`→Beetle PCE, `lynx`→Holani, `wonderswan`→Beetle Cygne. Each downloaded from the official nightly buildbot, hashed by hand with two independent methods (`sha256sum` + `certutil -hashfile`), and the internal DLL filename confirmed by actually listing the archive's contents. Catalog completeness does not imply interactive confirmation — only `nes` has been through a real Auto-Install click that launched a game; the other 14 are data-verified only
...
### Known Issues
- All 15 seed platforms have a data-verified `KnownEmulatorCore` entry, but only 1 (`nes`) has been through a real, interactive Auto-Install click — the install mechanism itself is proven end-to-end for that one (see ARCHITECTURE.md → ADR-14), the other 14 are not yet interactively confirmed (see DEVELOPMENT.md → Known Limitations)
