...
**Phase 2's catalog is complete for all 15 seed platforms, and the install mechanism is now interactively proven for 12 of them.** `EmulatorInstallerService`, the download-verification chain, and the catalog itself are all built, tested, and — for `nes` → FCEUmm plus 11 more platforms confirmed in a single later session (`lynx` → Holani, `wonderswan` → Beetle Cygne, `gb`/`gbc` → SameBoy, `genesis`/`sms`/`gamegear` → Genesis Plus GX, `gba` → mGBA, `pcengine` → Beetle PCE, `n64` → Mupen64Plus-Next, `nds` → melonDS DS) — confirmed working through a real interactive install that launched an actual game each time, per the user's own direct report. `snes` → Snes9x, `atari2600` → Stella, and `atari7800` → ProSystem went through the identical data-verification process (real buildbot source, double-hashed, archive contents inspected directly) but have no live Auto-Install click yet. The mechanism is generic and already proven against 12 platforms, so this is a narrowing, lower-risk gap, not an unknown — but it's still open for those 3, not claimed as closed. What's left for a fully-proven Phase 2 is exercising the orchestration against those last 3, not sourcing more data. See ARCHITECTURE.md → ADR-11/ADR-14 → Consequences, DEVELOPMENT.md → Known Limitations.

**Two things were investigated during that same interactive session and confirmed with real evidence, not assumed.** First: several Sega ROMs (Genesis/SMS/Game Gear) appeared to use "the same core" during Auto-Install — confirmed as the documented design (ARCHITECTURE.md → ADR-11's 2026-08-04 update), not a system-detection bug: `KnownEmulators.json` has three separate `KnownEmulatorCore` entries, one per platform, that all happen to point at the identical Genesis Plus GX binary/hash, because that core genuinely covers all three systems. Second: two real Atari 2600 ROMs (`.bin` extension) in the user's actual `%UserProfile%\Downloads\ROMS` folder went undetected. Following the Bug Investigation Process: the "weird casing" hypothesis was ruled out by direct code inspection (`RomScannerService`'s extension matching is fully case-insensitive), and the real cause was confirmed by inspecting `SeedSystems.json` directly — `atari2600` only lists `"a26"`, never `"bin"`, so the files fall through to the existing `unknown`-platform fallback (ADR-6), not a scanner bug. See Known Limitations below — not fixed yet, pending a decision on how to handle `.bin`'s ambiguity across other systems' disc images.
...
| **3 of 15 seed platforms (`snes`→Snes9x, `atari2600`→Stella, `atari7800`→ProSystem) have a data-verified `KnownEmulatorCore` entry but have never been through a real, interactive Auto-Install.** The other 12 (`nes`→FCEUmm, `gb`/`gbc`→SameBoy, `gba`→mGBA, `n64`→Mupen64Plus-Next, `nds`→melonDS DS, `genesis`/`sms`/`gamegear`→Genesis Plus GX, `pcengine`→Beetle PCE, `lynx`→Holani, `wonderswan`→Beetle Cygne) have all been clicked for real and produced a launchable game, per the user's own direct report — `nes` first, the other 11 in a single later session. All 15 entries went through the identical data-verification process (real nightly buildbot URL, `sha256sum`+`certutil`-matched hash, exact size, `CoreFileName` confirmed by directly listing the archive's contents); `dotnet test Bridge.slnx -c Release` passes (no placeholder values anywhere in the manifest) — the Release guard test doesn't count per-platform coverage or interactive confirmation, it only rejects placeholder sentinel values on whatever entries exist. **Data-verified and interactively confirmed remain two different claims here on purpose** — that same double-hash/content-inspection process alone already missed one real bug once (RetroArch's own `ExecutableRelativePath`, ADR-11's 2026-08-03 update). | Per-platform core sourcing/verification is done for all 15, and the orchestration (ADR-14) is now proven against 12 of them; what's left for the remaining 3 is exercising a real Auto-Install click, not sourcing more data. | Click "Auto-Install" for `snes`, `atari2600`, and `atari7800` against a real ROM each, the same way the `nes` bug and the `SettingsViewModel` staleness bug were found — treat any failure with the same Bug Investigation Process used for those two, not a one-off patch. |
| **`RomScannerService` doesn't detect Atari 2600 ROMs distributed with the common headerless `.bin` extension** — only `.a26` is in `SeedSystems.json`'s `atari2600.Extensions`. Confirmed with real evidence, not assumed: found via two real `.bin` files (`Donkey Kong (USA).bin`, `Pac-Man.bin`, both genuine Atari 2600 titles) sitting in the user's actual `%UserProfile%\Downloads\ROMS` folder. The "weird casing" hypothesis was tested and ruled out by direct code inspection (`RomScannerService.cs`: extension matching uses `ToLowerInvariant()` and `BuildExtensionMap` uses `StringComparer.OrdinalIgnoreCase`, fully case-insensitive). These files aren't silently dropped — they fall through to the existing `unknown`-platform fallback (ADR-6) and are scanned/persisted there, which is presumably why they read as "not detected" to the user (no platform, no emulator, easy to miss in the grid). | `.bin` was never added to `atari2600`'s extension list when the seed data was authored (ARCHITECTURE.md → ADR-7) — not a scanner bug, a data gap. Not yet fixed: `.bin` is genuinely ambiguous across other systems' disc images (e.g. PC Engine CD/PSX-style `.bin`/`.cue` pairs), so extending the list needs a real decision about scope, not a reflexive one-line addition. | Pending a decision on whether/how to add `.bin` to `atari2600.Extensions` without over-matching future disc-based platforms — see ARCHITECTURE.md → ADR-14's 2026-08-04 update for the full investigation. |
| No UI action exists to remove a `Game` from the library. Phase 1 only ever marks a `Game` `IsMissing = true` (ADR-6) — deliberate, so a temporarily-unavailable folder never silently loses data — but there's genuinely no way to get rid of an entry once it's confirmed gone for good, e.g. the leftover `.sav`-as-Game entry from before the companion-files fix (ADR-13), which self-healed to `IsMissing` but still sits in the grid forever. Found during real interactive use, not designed in from the start. | Never in scope for Phase 1 (`PLAN.md` doesn't list it as an FR); surfaced as a real gap only once actual library data accumulated real "should be gone" entries. | None today — manually deleting the row from `bridge.db` is the only option. Tracked as a Phase 2 backlog candidate in `PLAN.md`. |
