...
**Never bind `Image.Source` directly to a file-path string.** WPF's implicit `string` → `ImageSource` conversion creates a `BitmapImage` using its own default caching, which is keyed by URI and lives at the *process* level — completely separate from `ImageCacheService`'s on-disk file cache. If a file at a given path is ever deleted and rewritten with different content (a "Remove from Library" followed by the same game reappearing in a later rescan; any future feature that regenerates a cached image in place), WPF can keep serving the first bitmap it ever decoded for that path, even though the path string itself — and the file on disk — are both correct. This produced a real, hard-to-diagnose bug: the ViewModel and the XAML binding were both provably correct (confirmed with real, targeted instrumentation, not just re-reading the code), yet the wrong image rendered — because WPF's own cache, a layer neither of those two things touches, was stale. See ARCHITECTURE.md → ADR-23 (Update) for the full investigation.

**Always route image bindings through a converter that constructs the `BitmapImage` explicitly with `BitmapCreateOptions.IgnoreImageCache`** (see `Converters/CachedImagePathConverter.cs`) instead of relying on the implicit conversion. `ImageCacheService`'s own file-level cache is the only caching layer this project wants — a second, WPF-managed one on top of it, invisible in code and undocumented until this investigation, is a footgun for any feature that ever rewrites a previously-cached image file at the same path.
...
| `Resources/KnownEmulators.json` requires recurring manual maintenance because the libretro nightly core channel rebuilds at high frequency — confirmed by **three** real incidents in one working session on 2026-08-02 (ARCHITECTURE.md → ADR-11, 2026-08-02 updates): first, 11 of 15 core entries had drifted from a `2026-07-31` pin (by a few bytes each, caught by the size guard); then `stella` drifted *again*, landing on the exact same compressed size as its own fresh pin but genuinely different content, caught by the SHA256 guard instead; then, reported as "some emulators have this problem" rather than one core, a full sweep of all 15 catalog entries found 3 more (`fceumm`/`snes9x`/`mgba`) with the same same-size/different-hash pattern. **Tally: 15 of 15 catalog core entries drifted from their pin at least once within one session** — only the versioned/stable RetroArch frontend never moved. A hand-curated, hand-recaptured manifest cannot keep pace with a channel that rebuilds this often. Bridge's two-layer verification (size tolerance + exact hash, see ADR-11) correctly rejects every stale pin rather than installing something unverified — so this limitation costs availability (a real install fails until someone notices and recaptures), never safety. | Recapturing today is a full manual pass every time: a real `HEAD`/download, independent double-hashing (`sha256sum` + `certutil -hashfile`), and an archive-contents check, repeated per affected core, on top of first noticing and diagnosing the failure. The third incident showed reactive, one-core-at-a-time investigation doesn't scale either — a full-catalog sweep (all entries at once) was needed to find all 3 new mismatches instead of just the one reported. | **Proposed, not built, priority escalated (see `PLAN.md` → Roadmap):** a maintainer-only "manifest drift check" tool (e.g. a small script or `dotnet run` target under a `tools/` folder, never shipped in `Bridge.exe`) that runs the same `HEAD`+double-hash procedure across every `DownloadUrl` in the catalog on demand, and reports a diff of which entries drifted — turning "investigate a failure from scratch" into "run a check, review a diff." Deliberately keeps a human approving every hash change before it's committed — auto-accepting a new hash without review would defeat the actual security property ADR-11's checksum provides (protection against a compromised source), so this speeds up re-verification, it doesn't remove it. No longer tracked as a low-priority Phase 3 "someday" item — three real incidents in one session moved it to next-priority, ahead of further feature work. |
