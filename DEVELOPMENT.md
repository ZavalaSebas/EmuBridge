...
| `Resources/KnownEmulators.json` requires recurring manual maintenance because the libretro nightly core channel rebuilds at high frequency — confirmed twice in the same ~2-hour working session on 2026-08-02 (ARCHITECTURE.md → ADR-11, 2026-08-02 updates): first, 11 of 15 core entries had drifted from a `2026-07-31` pin (by a few bytes each, caught by the size guard); then, within that same session, `stella` drifted *again* — a second rebuild landed on the exact same compressed size as the first pin but genuinely different content, caught by the SHA256 guard instead. This isn't a one-off; a hand-curated, hand-recaptured manifest structurally cannot keep pace with a channel that rebuilds this often. Bridge's two-layer verification (size tolerance + exact hash, see ADR-11) correctly rejects every stale pin rather than installing something unverified — so this limitation costs availability (a real install fails until someone notices and recaptures), never safety. | Recapturing today is a full manual pass every time: a real `HEAD`/download, independent double-hashing (`sha256sum` + `certutil -hashfile`), and an archive-contents check, repeated per affected core, on top of first noticing and diagnosing the failure. Nothing today shortens that loop — every occurrence so far has started from a user-reported install failure and gone through the full Bug Investigation Process before a fix landed. | **Proposed, not built:** a maintainer-only "manifest drift check" tool (e.g. a small script or `dotnet run` target under a `tools/` folder, never shipped in `Bridge.exe`) that runs the same `HEAD`+double-hash procedure across every `DownloadUrl` in the catalog on demand, and reports a diff of which entries drifted — turning "investigate a failure from scratch" into "run a check, review a diff." Deliberately keeps a human approving every hash change before it's committed — auto-accepting a new hash without review would defeat the actual security property ADR-11's checksum provides (protection against a compromised source), so this speeds up re-verification, it doesn't remove it. See `PLAN.md` → Phase 3 for the tracked item. |
