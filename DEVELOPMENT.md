...
**Phase 2's catalog is now complete for all 15 seed platforms; the install mechanism is interactively proven for exactly one of them (`nes`).** `EmulatorInstallerService`, the download-verification chain, and the catalog itself are all built, tested, and — for `nes` → FCEUmm specifically — confirmed working through a real interactive install that launched an actual game. The other 14 platforms (`snes` → Snes9x plus 13 added in a single batch: `gb`/`gbc` → SameBoy, `gba` → mGBA, `n64` → Mupen64Plus-Next, `nds` → melonDS DS, `genesis`/`sms`/`gamegear` → Genesis Plus GX, `atari2600` → Stella, `atari7800` → ProSystem, `pcengine` → Beetle PCE, `lynx` → Holani, `wonderswan` → Beetle Cygne) went through the identical data-verification process (real buildbot source, double-hashed, archive contents inspected directly) but none has had its own live Auto-Install click yet. The mechanism is generic and already proven against `nes`, so this is a lower-risk gap than the first click was, not an unknown — but it's still open, not claimed as closed. What's left for a functionally complete, *proven* Phase 2 is exercising the orchestration against the other 14 platforms one at a time, not sourcing more data. See ARCHITECTURE.md → ADR-11/ADR-14 → Consequences, DEVELOPMENT.md → Known Limitations.
...
| **All 15 seed platforms have a `KnownEmulatorCore` entry in `KnownEmulators.json` now, but only 1 (`nes`) has ever been through a real, interactive Auto-Install.** Every entry (`nes`→FCEUmm, `snes`→Snes9x, `gb`/`gbc`→SameBoy, `gba`→mGBA, `n64`→Mupen64Plus-Next, `nds`→melonDS DS, `genesis`/`sms`/`gamegear`→Genesis Plus GX, `atari2600`→Stella, `atari7800`→ProSystem, `pcengine`→Beetle PCE, `lynx`→Holani, `wonderswan`→Beetle Cygne) is data-verified the same way: real nightly buildbot URL, `sha256sum`+`certutil`-matched hash, exact size, `CoreFileName` confirmed by directly listing the archive's contents. `dotnet test Bridge.slnx -c Release` passes (no placeholder values anywhere in the manifest) — the Release guard test doesn't count per-platform coverage, it only rejects placeholder sentinel values on whatever entries exist; the catalog-completeness claim itself is tracked here and in `ARCHITECTURE.md` → ADR-11, not enforced by a test. **Data-verified is not the same bar as interactively confirmed**: only `nes`'s auto-install has actually been clicked on a real machine and produced a launchable game (ADR-14). The other 14 core entries — including `snes`, added and used as the second data point in this same catalog work — are trusted on the strength of the same double-hash/content-inspection process, but that process alone already missed one real bug once (RetroArch's own `ExecutableRelativePath`, ADR-11's 2026-08-03 update), so "data-verified" and "known to actually work end-to-end" remain two different claims here on purpose. | Per-platform core sourcing/verification is done for all 15; what's left is exercising the orchestration (ADR-14) against each one via a real Auto-Install click, the same way `nes` was — not sourcing more data. | Click "Auto-Install" for each remaining platform against a real ROM, one at a time, the same way the `nes` bug and the `SettingsViewModel` staleness bug were found — treat any failure with the same Bug Investigation Process used for those two, not a one-off patch. |
