...
public class TheGamesDbService : ITheGamesDbService
{
    ...
    // Bridge's 15 seed platforms (genesis, wonderswan) map to TWO real TheGamesDB catalog entries
    ...
    private async Task<TheGamesDbGame?> SearchGameAsync(string gameName, string bridgePlatformId, string apiKey, CancellationToken ct)
    {
        if (!PlatformIds.TryGetValue(bridgePlatformId, out var candidateIds))
        {
            // Every one of Bridge's 15 seed platforms is mapped above - an unmapped platform means
            ...
            _logger.LogWarning(
                "No TheGamesDB platform mapping for Bridge platform '{PlatformId}'; skipping search rather than risk an unfiltered mismatch.",
                bridgePlatformId);
            return null;
        }
