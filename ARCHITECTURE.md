...
The remaining 14 of 15 seed platforms still have no `KnownEmulatorCore` entry at all. `KnownEmulatorsManifestTests.KnownEmulators_NoUnverifiedPlaceholdersInReleaseBuild` is compiled only under `#if RELEASE` (confirmed empirically: the SDK does define `RELEASE` for `-c Release`, `DEBUG` for `-c Debug`) — it now passes, since the one entry present carries no placeholder value; it would fail again the moment any future entry is added with `Config.UnverifiedManifestPlaceholder` still in it, matching the existing Release Checklist gate.
...
- ✅ 1 of 15 seed platforms (`nes`) now has a fully verified `KnownEmulatorCore` — `DownloadUrl`, `Sha256`, `ExpectedSizeBytes`, and `CoreFileName` all confirmed from the real distribution channel, not assumed
- ❌ **14 of 15 seed platforms still have no `KnownEmulatorCore` entry at all.** Phase 2's auto-detect/download feature is still **not functionally usable end-to-end** — no archive-extraction/install orchestration consumes any of this yet (a separate, not-yet-built step), and only one platform has a source to consume in the first place. Sourcing and verifying the remaining 14 cores (same process now proven for `nes`) is the next concrete step, not optional polish.
