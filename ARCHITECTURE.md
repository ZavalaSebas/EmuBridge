...
**Status:** Accepted (mechanism); manifest data fully verified for all 15 seed platforms, 12 of 15 interactively install-confirmed — see Consequences and ADR-14's 2026-08-04 update
...
- ✅ **12 of 15 seed platforms have now been through a real, interactive Auto-Install that succeeded end-to-end** — `nes` originally, plus 11 more confirmed in one session (see ADR-14's 2026-08-04 update). `snes`, `atari2600`, and `atari7800` remain data-verified only, not yet click-tested; the install mechanism itself (ADR-14) is proven, so this is a narrowing gap, not an open unknown.
...
- ❌ **3 of 15 seed platforms (`snes`, `atari2600`, `atari7800`) remain data-verified only** — no live Auto-Install click confirming they actually install and launch a game. See the 2026-08-04 update below for the other 12.
- ~~No visual/interactive confirmation that the "Auto-Install" button actually renders and behaves correctly in a real running window~~ — closed for 12 of 15 platforms, see the 2026-08-04 update below. **This is exactly the gap that surfaced ADR-11's `ExecutableRelativePath` bug** — every automated test used hand-built `.zip` fixtures matching the assumed (wrong) flat structure, so nothing caught the mismatch until a real click, on a real machine, against the real archive. That risk is now retired for `nes` plus the 11 platforms confirmed below; it remains open for the 3 still data-only.
