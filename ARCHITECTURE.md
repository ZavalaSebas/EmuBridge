...
- ❌ Bridge now embeds 2 third-party API keys rather than 1, each requiring separate manual registration and rotation coordination — see `DEVELOPMENT.md` → Known Limitations
...
- **ScreenScraper:** rejected for now, not permanently — genuinely the strongest match-quality candidate (hash/CRC ROM identification, something none of the title-search-only sources have), but its full quota requires a real per-end-user account Bridge cannot provision on the user's behalf, a materially different onboarding cost than an embedded project key; revisit if ROM-hash matching specifically becomes a real, felt need
