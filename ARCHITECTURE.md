...
The remaining 13 of 15 seed platforms still have no `KnownEmulatorCore` entry at all. `KnownEmulatorsManifestTests.KnownEmulators_NoUnverifiedPlaceholdersInReleaseBuild` is compiled only under `#if RELEASE` (confirmed empirically: the SDK does define `RELEASE` for `-c Release`, `DEBUG` for `-c Debug`) — it now passes with both real entries present (137 tests, Release), since neither carries a placeholder value; it would fail again the moment any future entry is added with `Config.UnverifiedManifestPlaceholder` still in it, matching the existing Release Checklist gate. Note this test only rejects placeholder sentinel values on whatever entries exist in the manifest — it has no assertion counting how many of the 15 seed platforms are covered; that count is tracked in prose here and in `DEVELOPMENT.md` → Known Limitations, not enforced by a test.
...
- ✅ 2 of 15 seed platforms (`nes` → FCEUmm, `snes` → Snes9x) now have a fully verified `KnownEmulatorCore` — `DownloadUrl`, `Sha256`, `ExpectedSizeBytes`, and `CoreFileName` all confirmed from the real distribution channel, not assumed
- ❌ **13 of 15 seed platforms still have no `KnownEmulatorCore` entry at all.** Sourcing and verifying the remaining 13 cores (same process now proven for `nes` and `snes`) is the next concrete step, not optional polish — the install orchestration itself (ADR-14) is proven end-to-end and consumes any verified entry immediately once added, no further mechanism work needed per core.
...
- ❌ Only 2 of 15 seed platforms can actually be auto-installed today (`nes`, `snes`) — this ADR proves the mechanism works, it doesn't populate the catalog; that's still `PLAN.md` → Timeline's next step.
- ❌ No visual/interactive confirmation that the "Auto-Install" button actually renders and behaves correctly in a real running window — covered by ViewModel-level tests (confirmation dialog, busy state, cancel, error display) and by the orchestration's own extensive tests, not by watching it on screen. Same category of gap as `PLAN.md`'s still-open interactive click-through pass. **This is exactly the gap that surfaced ADR-11's `ExecutableRelativePath` bug** — every automated test used hand-built `.zip` fixtures matching the assumed (wrong) flat structure, so nothing caught the mismatch until a real click, on a real machine, against the real archive.
