...
This document records architectural decisions made during the development of EmuBridge.
...
- ❌ Known to break for any emulator that launches via a wrapper/launcher process that exits before the real emulator does — EmuBridge would return control while the emulator is still running
...
- **Track by directory activity:** rejected as too heuristic/flaky for Phase 1, and solves a problem (no single trackable process) not yet confirmed to exist for the emulators EmuBridge targets first
...
**Context:**
`LibraryRepository` needs to persist EmuBridge's library (games, systems, emulator configs) locally. EmuBridge's NFR requires a single-file, self-contained `.exe`. Playnite (MIT-licensed, reviewed as prior art) uses LiteDB 4, split one physical file per collection (17 collections, matching its much larger taxonomy surface — genres, companies, tags, etc.), with an in-repo code comment justifying LiteDB v4 over v5 (disabling the memory cache, write speed) — not a LiteDB-vs-SQLite comparison.

**Decision:**
LiteDB (current 5.x, not Playnite's old v4 pin), a single `bridge.db` file with 2–3 named collections (not Playnite's one-file-per-collection split — EmuBridge has ~3 entities, not 17 taxonomies). Decisive factor, per official docs ([Create a single file for application deployment - .NET \| Microsoft Learn](https://learn.microsoft.com/en-us/dotnet/core/deploying/single-file/overview), "Native libraries"): SQLite's native interop (`Microsoft.Data.Sqlite` → SQLitePCLRaw) isn't bundled into a true single file by default — it ships as a separate file next to the `.exe` unless `IncludeNativeLibrariesForSelfExtract=true` is set, and even then it's extracted to `%TEMP%/.net` on every startup before the app can use it. LiteDB is pure managed code — no native interop, no extra packaging step.

**Consequences:**
- ✅ Genuinely single-file distribution, no extra packaging configuration needed
- ✅ Document model fits EmuBridge's small, mostly-lookup-shaped entity set better than normalizing into relational tables
...
1. **Required token check:** `ArgumentTemplate` must contain `{RomPath}` literally, or expansion throws `EmuBridgeException` — a template missing it would launch the emulator with no ROM argument (likely just opening its main menu), a silent functional failure even though the process technically starts.
2. **Unknown token check:** any `{Token}` in the template not found in the dictionary throws `EmuBridgeException` during expansion.

Auto-quoting: the evaluator inspects the template characters immediately surrounding each match — if the resolved value contains a space and isn't already wrapped in manual `"..."` in the template, it gets quoted; if the user already wrote `"{RomPath}"`, EmuBridge doesn't double-quote.
...
**Context:**
`MetadataService` needs a SteamGridDB API key to fetch box art. The original foundation document leaned toward a user-supplied key in Phase 1 ("simpler, no cost to the developer") versus an embedded key ("more plug & play, but risks shared rate-limiting and key exposure"), without fully spelling out why embedding is actually untenable for this specific project. EmuBridge is a public, open-source (GPL-3.0) repository distributing a single-file `.exe`.

**Decision:**
User-supplied key, confirmed — and reinforced with a sharper reason than the original framing: since EmuBridge's source and published binary are both public, an "embedded" key isn't actually securable at all — it's readable directly from the repo or trivially recoverable from the compiled `.exe`. This makes it a structural non-starter for a public OSS project, not just a cost trade-off. (A server-side proxy holding a key secret behind EmuBridge's own backend would sidestep this, but that's a different architecture entirely — not considered here, noted only as a hypothetical future path if ever pursued.)

Storage: `%LocalAppData%\EmuBridge\settings.json`, with the key DPAPI-encrypted via `System.Security.Cryptography.ProtectedData` (`DataProtectionScope.CurrentUser`) — confirmed available as a current NuGet package ([Microsoft Learn](https://learn.microsoft.com/en-us/dotnet/api/system.security.cryptography.protecteddata), [NuGet](https://www.nuget.org/packages/System.Security.Cryptography.ProtectedData)), Windows-only (fine — EmuBridge is Windows-only), ties decryption to the specific Windows account.
...
**Consequences:**
- ✅ No shared rate-limit risk across all EmuBridge installs, and no key-exposure liability for the developer — each user's key is theirs alone
...
Disc-based platforms (PS1, PS2, Saturn, Dreamcast, GameCube, Wii, PSP, and similar) are **not** in the seed. Their common dump formats — `.iso`, `.bin`, `.cue`, `.chd`, `.gdi`, `.cdi`, `.wbfs`, `.pbp`, `.cso` — are shared across many different disc-based systems, and Phase 1's extension-only matching (ADR-3) has no mechanism to disambiguate which platform a given `.iso` actually belongs to. This is a **deliberate exclusion by Phase 1's detection mechanism, not an accepted limitation** — unlike ADR-1 (TrackingMode), where Option A was accepted because the more robust alternative (Job Objects) has real implementation cost that wasn't yet justified, there is no comparable cost being avoided here. The user can add any disc-based platform manually at any time through the same editable schema, consciously choosing which extension to risk assigning to it — EmuBridge's default posture for these platforms is "not seeded, so no default ambiguity," not "seeded with broken detection because building it right was too expensive."
...
- **Replicate Playnite's decode-time image resizing (no disk cache of resized bitmaps):** rejected — EmuBridge's box art is fetched over the network, unlike Playnite's local imports, so re-decoding on every display wastes the original download; already the conclusion from the Playnite research pass
...
**Context:**
Phase 2's "automatic emulator detection/download" (`PLAN.md`) needs EmuBridge to download and later execute third-party binaries (RetroArch + libretro cores) it didn't build — a materially different trust boundary than Phase 1's `ImageCacheService`, which only ever caches inert image bytes. RetroArch alone covers all 15 seed platforms via different cores, which the Phase 1 `EmulatorConfig` schema (1:1 with `Platform`, reviewed and deliberately scoped down from Playnite's `Emulator 1—* Profile *—* Platform` shape back in ADR-3) cannot express — one physical install needs to back many per-platform launch configs, not one. This ADR covers the schema migration and the download-verification mechanism; it does not build the actual install/extract orchestration (a future ADR) or a real per-platform core catalog (see Consequences).

**Decision — Emulator/EmulatorProfile split:** `EmulatorConfig` is replaced by `Emulator` (a physical install: `ExecutablePath`, `InstallSource` (`UserProvided`/`EmuBridgeManaged`), `InstalledSha256`, optional `KnownEmulatorId`) and `EmulatorProfile` (per-platform launch config: `EmulatorId`, `PlatformId`, `ArgumentTemplate`) — realizing the split Decision #2 in `PLAN.md` already promised as a "mechanical one-time data migration, not a contract change." `EmulatorService.SaveProfileAsync` finds-or-creates the `Emulator` by `ExecutablePath` (case-insensitive) before upserting the `EmulatorProfile`, so two platforms pointed at the same executable share one `Emulator` row — the actual scenario RetroArch creates, not a hypothetical. `ResolvedEmulatorProfile` (the `Emulator`+`EmulatorProfile` join) is the only shape `LaunchService`/`SettingsViewModel` see; `EmulatorService` stays the sole place that knows the split exists, unchanged from ADR-3's original promise.
...
|---|---|---|---|
| `gb`/`gbc` | SameBoy | `sameboy_libretro.dll.zip` | "extremely accurate"; built-in boot ROMs remove a firmware-sourcing step from EmuBridge's install flow entirely |
...
- **Fetch the KnownEmulator manifest live from a EmuBridge-controlled backend at runtime:** rejected — adds server infrastructure EmuBridge doesn't have, and doesn't change the trust story (EmuBridge would still author the manifest); embedding it in the repo ties the pinned hash/URL to the same commit that ships the app code
- **Auto-install via RetroArch's own installer (`RetroArch-Win64-setup.exe`) instead of the portable `.7z`:** rejected for the eventual install step — needs UAC elevation and more failure surface, and fights EmuBridge's own single-file-no-installer philosophy; noted here because it shaped which artifact was verified (the portable `.7z`, not the installer)
...
**Context:**
The `v0.1.0` GitHub Release's `EmuBridge.exe` did not open at all for a real user — no window, no dialog, no visible error, on a machine where it had never run before. Following `DEVELOPMENT.md` → Bug Investigation Process: the first specific hypothesis (an exception thrown during `App()`'s constructor, before `DispatcherUnhandledException` is wired — e.g. `LibraryRepository` failing to open LiteDB because `%LocalAppData%\EmuBridge\` doesn't exist yet on a clean machine) was tested with temporary file-based diagnostic logging and a simulated clean-machine run, and was **ruled out by direct evidence** — the full startup sequence completed with zero exceptions logged.

The real cause required questioning an assumption made when the release was originally cut: that `dotnet publish -p:PublishSingleFile=true` produces one file and nothing else worth checking. It doesn't, for a WPF app specifically. A full listing of a fresh publish output directory (not just `EmuBridge.exe`'s own byte size, which is all that had been checked before) showed `EmuBridge.exe` sitting next to `EmuBridge.pdb` and five native interop DLLs WPF depends on (`D3DCompiler_47_cor3.dll`, `PenImc_cor3.dll`, `PresentationNative_cor3.dll`, `vcruntime140_cor3.dll`, `wpfgfx_cor3.dll`) — `PublishSingleFile` bundles managed assemblies into the `.exe`, but leaves native (non-managed) libraries as loose sibling files by default. `gh release create v0.1.0 publish/EmuBridge.exe` uploaded only the named `.exe`, never these five files — the release asset was incomplete from the moment it was published, not corrupted afterward. Confirmed directly, not assumed: copying only `EmuBridge.exe` into an empty folder and running it reproduced the exact symptom — `System.DllNotFoundException` thrown from inside WPF's own native window-subclassing code (`MS.Win32.HwndSubclass`) before a single line of `App()`'s constructor ran (the diagnostic log file was never even created), an unhandled exception written only to stderr — invisible to a user double-clicking from Explorer, exactly matching "doesn't open at all."

**Decision:** `EmuBridge.csproj` sets `<IncludeNativeLibrariesForSelfExtract>true</IncludeNativeLibrariesForSelfExtract>`, which bundles those native libraries into the single-file `.exe` too, instead of leaving them as required sibling files.

**Trade-off, stated explicitly, not left implicit:** with this flag on, the bundled native libraries are extracted to `%TEMP%/.net` on every startup before the app can use them — the exact "startup extraction cost" already weighed and rejected for SQLite back in Decision #1 / ADR-3 (`LibraryRepository`'s storage choice), where it was one reason LiteDB (pure managed code, no native interop) won over `Microsoft.Data.Sqlite`. It's being accepted here for a different reason: WPF itself requires these native libraries to run at all — there is no "zero extraction" alternative available the way there was for storage (LiteDB vs. SQLite was a real choice; a WPF app either ships these DLLs as loose files or extracts them from the bundle, no third option). This was already true before the fix — the DLLs always had to end up on disk somewhere; the flag changes where they come from (bundled in `EmuBridge.exe`, extracted to `%TEMP%/.net`) rather than whether the cost exists at all.

Fix confirmed with the same reproduction method that found the bug, not assumed from the flag being set: republished with the flag on, copied only the resulting `EmuBridge.exe` into an empty folder (no sibling DLLs at all), ran it — it opened normally.

**Consequences:**
- ✅ `EmuBridge.exe` alone, with nothing else in the folder, now actually matches what "single-file" was always supposed to mean — confirmed by the same isolated-folder reproduction that found the bug, not just by re-running the publish command and assuming success
...
Extensions were confirmed against the two emulators actually relevant to EmuBridge today — mGBA (configured and used in the interactive test above) and RetroArch (Phase 2's install target) — not assumed or carried over from the user's initial guess unverified:
...
- **A dedicated "Manage Library" view/window:** rejected — too much surface for one action; nothing in EmuBridge today has a secondary view, and this doesn't justify being the first
...
**Context:**
`v0.3.0`'s two confirmed Roadmap items (`PLAN.md` → Roadmap) were a core picker UI (for platforms with more than one known-good `KnownEmulatorCore` candidate) and offering Auto-Install inline when `LaunchService` returns `NoEmulatorConfigured`, not just from Settings — both deliberately deferred during Phase 2's build-out (ADR-14) until the mechanism had more proven ground behind it. Investigated before designing either: `KnownEmulators.json` has exactly one `KnownEmulatorCore` per platform, for all 15 seed platforms, confirmed by inspecting the manifest directly — every platform's core was already narrowed to a single best choice during ADR-11's curation (e.g. Snes9x over bsnes-mercury for `snes`). The core picker's premise — a platform with more than one real candidate — doesn't exist anywhere in EmuBridge's data today.
...
Two problems were reported after real use: (1) Adventure (Atari 2600) has a verified vertical grid on SteamGridDB, but EmuBridge didn't show it; (2) box art outside Big Picture looked visibly stretched. Followed the Bug Investigation Process on both — real evidence before any fix.
...
**Decision — direct delete on `DeleteEmulatorProfileForGameAsync`, no "still referenced" check:** explicitly investigated whether a per-game `EmulatorProfile` row could ever be shared or referenced elsewhere, the same way `BoxArt`'s cached image files are (deduped by URL hash, so one file can be referenced by multiple `BoxArt` rows and needs a "still referenced by another game?" check before deletion in `DeleteGameAsync`). Confirmed by exhaustive `grep` across `EmuBridge/Models`, `EmuBridge/Services`, `EmuBridge/ViewModels` for every `EmulatorProfile` reference: no other entity stores `EmulatorProfile.Id` as a foreign key, and the `(PlatformId, GameId)` key with a real `GameId` uniquely identifies exactly one game's row — it can never be shared the way a deduped cache file can. `MainViewModel.DeleteGameAsync` calls `DeleteEmulatorProfileForGameAsync(game.Id)` unconditionally, right alongside the existing `BoxArt` cleanup, with no reference check — different from `BoxArt`'s pattern deliberately, not by oversight.
...
2. **Getting a fix to users faster than EmuBridge's own release cadence** — today, even a same-day manifest fix only reaches users on the next tagged version.
...
Lives under `tools/ManifestDriftCheck/` — a plain console project (`net10.0-windows`, matching `EmuBridge.Tests`' own TFM so it can `ProjectReference` `EmuBridge.csproj` and reuse `KnownEmulator`/`KnownEmulatorCore` directly, avoiding a second, driftable copy of the schema), referenced only from the workflow, never documented as a maintainer command, never shipped in `EmuBridge.exe`'s publish output (`release.yml` publishes `EmuBridge/EmuBridge.csproj` alone).
...
- ✅ A same-day catalog fix can reach a running EmuBridge instance without waiting for a new tagged release — closes the actual gap the 2026-08-02 incidents exposed, not just the symptom
- ✅ Merge stays a human decision at every layer — the drift-check PR, and by extension what a live-fetching EmuBridge instance will ever pick up, both require a click, never automatic (see the auto-merge idea explicitly deferred in `PLAN.md` → Speculative Ideas, pending real evidence)
...
- ❌ A given `EmuBridge.exe` build's behavior is no longer fully reproducible from its own version alone — a real, named trade-off (see the reconciliation above), not an oversight
- ❌ The "silent on failure" exception for `IManifestUpdateService` is a deliberate deviation from EmuBridge's own standing rule — documented here and in the class's own doc comment specifically so it doesn't read as an inconsistency later
...
**Decision — the two external fetches EmuBridge now makes stay deliberately separate, not merged into one list:** `IManifestUpdateService` (ADR-25) fetches `Config.ManifestUpdateUrl` — `raw.githubusercontent.com`, a small JSON *description* of what to download, never executed itself — while `DownloadVerificationService` fetches and extracts *executable content* that gets run. `Config.ManifestUpdateUrl` is already itself a single fixed, compiled URL with no variability to constrain — there is no manifest-driven redirection possible for that fetch, so no allow-list concept even applies to it. Adding `raw.githubusercontent.com` to `AllowedDownloadHosts` would blur a distinction worth keeping sharp: one list gates what EmuBridge is willing to *run*, the other fetch has no list because it has no variable destination in the first place.

**Decision — adding a new host is a source-code change, not a data change, on purpose:** expanding `AllowedDownloadHosts` means editing `Config.cs` and shipping a new EmuBridge version through the normal build+test+review+commit process — never something the drift-check bot (ADR-25) can do, even indirectly. That bot only ever rewrites `Sha256`/`ExpectedSizeBytes`/`CapturedAt` on entries whose `DownloadUrl` already points at an already-trusted host; it has no path to introduce a new host into the allow-list. "Should EmuBridge trust downloads from a brand-new domain at all" is a categorically bigger decision than "is this pinned hash still correct" and deliberately doesn't get to ride through the same lightweight, largely-automated review the routine drift-check PRs do.
...
**Consequences:**
- ✅ Closes the actual gap ADR-25 left open: even a fully compromised `main` (and therefore a compromised live-fetched manifest) cannot direct EmuBridge to download and run a binary from anywhere outside a small, hardcoded, source-reviewed list — the hash check alone could never have provided this
- ✅ Rejected sources never generate a network request — a stronger property than catching the problem after downloading
- ✅ The distinction between "what EmuBridge is willing to run" (allow-listed, hardcoded) and "what EmuBridge is willing to read as data" (the manifest, JSON only, never executed) stays explicit rather than implicit
...
- ❌ A genuinely new, legitimate download source (a different libretro mirror, a standalone emulator from a different vendor — see `PLAN.md` → Speculative Ideas) requires a EmuBridge source change and a new release before it can be used, not just a manifest update — an intentional friction, not an oversight
...
- **`wonderswan`:** no folder exists in the real `libretro/libretro-database` `cht/` directory (confirmed by listing it directly, not guessed from a naming convention) — a gap in the upstream source, not something EmuBridge's own code can work around. `LoadCheatsAsync` reports `PlatformNotSupported` for it rather than attempting a fetch that would always fail.

Any of these three, if it turns out wrong in real use, fails the same visible way bug 2 above did — RetroArch silently not finding what EmuBridge wrote — not a crash or data loss. Revisit with the same real-evidence standard (source + empirical confirmation) if a real report comes in.

**Alternatives considered:**

- **Curate/bundle cheat content the same way `KnownEmulators.json` curates emulators:** rejected — cheat databases are large, per-game, and change independently of EmuBridge's own release cycle; fetching on demand from the same source RetroArch itself uses keeps EmuBridge's own maintenance surface at zero
- **Force `cheat_apply_after_load = true` globally via a direct one-time write to the user's `retroarch.cfg`:** rejected — this is exactly the "global forced default" outcome the approved design explicitly ruled out; it would also affect every game/core, not just ones EmuBridge manages cheats for
- **Keep `--appendconfig`, and instead force `config_save_on_exit = false` for EmuBridge-launched sessions to prevent the leak:** rejected — this would silently discard *any* other setting change the user makes during that RetroArch session (video/audio tweaks, remaps), a much larger unwanted side effect than the one it would fix
- **Have EmuBridge track and actively revert the leaked `apply_cheats_after_load`/`cheat_database_path` values on every launch:** rejected — treats a symptom of using the wrong mechanism instead of using RetroArch's own real session-scoped mechanism, which needs no cleanup step at all
- **Leave mechanism 1 on the `LIBRETRO_CHEATS_DIRECTORY` env var, accepting the leak as a known limitation since it only affects RetroArch's own cheat-browsing UI between EmuBridge-driven launches, not EmuBridge's own cheat management:** rejected after explicit review — the fix is the same pattern already built for mechanism 2 (same file, same targeted-line-patch discipline), so there was no real cost/complexity reason to leave a known, real, root-caused leak unfixed for the version cut

---

### ADR-28: TheGamesDB as a second metadata source — description and screenshots, chosen against real evidence, not documentation alone
...
**Context:**
`PLAN.md` → Phase 3's confirmed build order, item 2 ("metadata source decision"), was scoped specifically because SteamGridDB (EmuBridge's only integrated source until now) is confirmed, twice over (ADR-19, ADR-22), to have no description, no screenshots, and no video/similarity data — an art-asset database, not a game-metadata one. Real candidates were inventoried against primary sources only (each API's own official docs, the same standard already applied to RetroArch's source in ADR-27) rather than third-party summaries: IGDB, TheGamesDB, RAWG, ScreenScraper, and LaunchBox Games Database. A first coverage pass tested all three leading candidates' *public websites* (not their authenticated APIs) against a curated 12-game flagship sample (Mario/Zelda/Pokémon-tier titles) and got 100%/100%/100% across IGDB, TheGamesDB, and ScreenScraper alike — explicitly flagged as insufficient evidence to decide anything, since a sample chosen to be easy proves nothing about real-world coverage.

**Decision — TheGamesDB chosen, coexisting with SteamGridDB rather than replacing it:** SteamGridDB keeps owning box art (already proven reliable in production since ADR-8); TheGamesDB is additive, filling the description/screenshot gap SteamGridDB structurally can't. IGDB was ruled out over a real, unresolved conflict found reading its own Business FAQ against the Twitch Developer Services Agreement it says governs it: the FAQ says "we prefer if you store and serve the data" (EmuBridge already caches box art locally, would extend this to descriptions/screenshots), but the DSA's own text prohibits "re-syndication and re-distribution" beyond a 24-hour cache with no IGDB-specific carve-out found anywhere in either document. RAWG couldn't be evaluated at all — `rawg.io`, including its API, was down (Cloudflare 522) for the entire research window, confirmed independently via `WebFetch` and live browser navigation, not a tooling artifact; deciding on it would have meant filling the gap with unverified secondary-source numbers, which was explicitly rejected. ScreenScraper has a real, structural advantage none of the others have (hash/CRC-based ROM identification, not just title search) but requires a per-end-user ScreenScraper account for its full, reliable quota — a real onboarding-friction cost with no code-side mitigation, since EmuBridge (and this session's own tooling) cannot create accounts on a user's behalf. LaunchBox Games Database has no real API at all, confirmed via LaunchBox's own open feedback-board request for one — only a static bulk XML mirror, with no hash matching.

**Decision — real coverage confirmed via the authenticated API, not the public website, against a deliberately hard sample:** a 15-game sample (2 known SNES/NES ROM hacks, 3 real region-exclusive titles — Mr. Gimmick, Bahamut Lagoon, Pulseman, all individually verified against Wikipedia after two other initially-assumed "exclusives" turned out to have real US releases and were dropped — and 10 B-tier titles spanning platforms never tested before: PC Engine, Atari Lynx, Atari 7800, WonderSwan, Game Gear, Atari 2600, SMS, GBA, GBC, NDS) was run against TheGamesDB's live `/v1` API using a real, rotated developer key. Result: 13/15 clean, platform-verified hits with real overview text and real images (86.7%, or 92.9% excluding one inconclusive case caused by an imprecise search title, not a TheGamesDB gap) — a real, confirmed miss rate, not the flagship sample's misleading 100%. The one clean miss (Kaizo Mario World) confirmed TheGamesDB doesn't catalog ROM hacks — an official-release database, not a homebrew/hack tracker, a real and permanent gap, not a bug.

**Decision — matching requires a platform filter, confirmed empirically, not assumed from the docs:** an unfiltered by-name search for "Ninja Gaiden" (targeting the obscure Atari Lynx port) returned 20 results spanning Wii/Xbox/Genesis/Arcade/TurboGrafx entries with no Lynx result even in the top 5 — the real Lynx entry existed (`id=22436`, real overview, 7 real images, confirmed once filtered) but a naive "take the first result" approach would have silently matched the wrong platform's game. `TheGamesDbService.PlatformIds` maps all 15 of EmuBridge's seed platforms to their real, `/Platforms`-endpoint-confirmed TheGamesDB ids — including a real complication found along the way: TheGamesDB catalogs `genesis` and `wonderswan` under **two** separate ids each (Sega Genesis=18 / Sega Mega Drive=36; WonderSwan=4925 / WonderSwan Color=4926) for what's the same hardware under different regional names, so both are tried, primary first, before concluding a game isn't catalogued at all.

**Decision — data model: extend `BoxArt`, same precedent ADR-19 already set for `ReleaseYear`:** `Description`, `DescriptionStatus` (reusing `BoxArtStatus`), `DescriptionRateLimitResetUtc`, `ScreenshotLocalPaths`. One shared status covers the whole per-game TheGamesDB lookup (not independent per-field statuses like `Status`/`VerticalStatus`'s box-art orientations) — screenshots degrade gracefully within a `Cached` status instead (a game can have a real description and zero screenshots, confirmed as a real, non-hypothetical case during interactive testing — "a veces solo trae la descripción").

**Decision — fetched on demand when `GameDetailWindow` opens, never during the library scan:** unlike box art (needed immediately for every tile in the grid), description/screenshots are only ever seen if the user opens a specific game's detail view. The real, confirmed public-key allowance is **~1000 requests/month** (`remaining_monthly_allowance` observed directly in a live response) — pre-fetching for an entire library during a scan would burn most or all of that in one pass for data nobody may ever look at. The real cost of this choice, confirmed during interactive testing: opening a game's detail view for the first time has a visible load delay (2 sequential HTTP calls plus however many screenshot downloads) — an accepted, deliberate tradeoff, not a defect.

**Decision — auth: same embedded-project-key DPAPI pattern as SteamGridDB (ADR-5):** `SettingsService.SetTheGamesDbApiKeyAsync`/`GetTheGamesDbApiKeyAsync`, `ProtectedData.Protect`/`Unprotect`, `DataProtectionScope.CurrentUser` — the exact same routine, refactored into shared `Encrypt`/`DecryptOrNull` helpers rather than duplicated a second time. TheGamesDB's own key-type distinction (a "public" key rate-limited per end-user IP, meant to be embedded in a distributed client, versus a "private" key for server-side mirroring) confirms this is the intended usage shape for EmuBridge's case, not a workaround.

**Decision — HTTP 403 disambiguation is a real, evidence-grounded heuristic, not a fully proven one:** TheGamesDB returns HTTP 403 for both an invalid API key and (per its documented schema) a real rate limit. A live call against a just-rotated, now-invalid key captured the real invalid-key shape: `{"code":403,"status":"Invalid API key was provided.","allowance_refresh_timer":0}`. A real rate-limit response was never actually observed (the research key never hit its ~1000/month cap) — `TheGamesDbService.ThrowIfSpecialStatusAsync` treats `allowance_refresh_timer > 0` as the signal for a genuine rate limit versus an auth failure, documented in code as inferred from the schema, not copied from a live rate-limited response. Flagged for revisiting if it's ever seen to misclassify in practice.

**A real security incident, corrected within the session, not after the fact:** the TheGamesDB developer key was shared directly in chat during this research. The harness's own permission-recording mechanism wrote the full literal command (key included) into `.claude/settings.local.json` — a real file on disk, though never committed (covered by a pre-existing global `.gitignore` rule, confirmed via `git log` returning empty for that path). Found via an explicit repo-wide grep sweep (the same discipline already used for `LIBRETRO_CHEATS_DIRECTORY` in ADR-27), removed immediately, and the key was rotated as a direct precaution — the old key was confirmed dead via a real API call (`403 Forbidden`, `"Invalid API key was provided."`) before the new one was persisted, DPAPI-encrypted, into the real local `settings.json`.

**Consequences:**
- ✅ The detail panel now shows real, verified descriptions and screenshots for a large majority of a deliberately hard sample (not just flagship titles) — confirmed both via a live API test and real interactive use in the running app
- ✅ Coexists cleanly with SteamGridDB — confirmed by a dedicated test (`FetchDescriptionAndScreenshotsAsync_ExistingBoxArtCoverArt_IsNotClobbered`) that a TheGamesDB fetch never touches SteamGridDB-owned `BoxArt` fields
- ✅ The Genesis/Mega Drive and WonderSwan/WonderSwan Color id-fallback logic is covered by a dedicated test reproducing the real scenario, not just asserted in a comment
- ✅ Missing/failed cases render the same explicit, never-silent "Description: not available" text ADR-19 already established, extended with a distinct rate-limit message when TheGamesDB's own reset-timer data is available — something SteamGridDB's own 429 handling can't do (see `DEVELOPMENT.md` → Known Limitations)
- ❌ TheGamesDB does not catalog ROM hacks at all — confirmed, not guessed; games sourced from the ROM-hacking community will always show "not available" here regardless of retry
- ❌ First-time detail-view opens have a real, visible load delay — an accepted cost of the on-demand fetch design, not yet mitigated with any loading indicator in the UI
- ❌ The HTTP 403 rate-limit/auth-failure disambiguation is inferred from the response schema, not confirmed against a real rate-limited response — see the heuristic decision above
- ❌ EmuBridge now embeds 2 third-party API keys rather than 1, each requiring separate manual registration and rotation coordination — see `DEVELOPMENT.md` → Known Limitations

**Known gaps (not resolved, flagged rather than guessed):**
- **ROM hacks have no coverage on this source, ever** — confirmed via a real miss (Kaizo Mario World) during the hard-sample test, not assumed. No workaround exists on TheGamesDB's side; would need a different, hack-aware source (none evaluated here covers this either) to close.
- **The true rate-limited 403 response shape is unconfirmed** — see the heuristic decision above.
- **`atari2600`/`lynx`/`wonderswan` TheGamesDB platform ids were taken from the live `/Platforms` list directly (high confidence), unlike the medium-confidence RetroArch core-name guesses ADR-27 flagged for the same three platforms** — these are two independent identification systems (TheGamesDB catalog id vs. RetroArch core `library_name`) and shouldn't be conflated; this ADR's ids are not the source of ADR-27's medium-confidence flag.

**Alternatives considered:**

- **IGDB:** rejected — real, unresolved conflict between IGDB's own caching-friendly FAQ and the Twitch DSA governing it (see decision above); free real-time API also caps at 4 requests/second with no documented higher tier, versus TheGamesDB's simpler monthly-allowance model
- **RAWG:** rejected — primary source (site and API both) was down for the entire evaluation window; deciding on unverified secondary-source claims was explicitly rejected as a standard this session already applies everywhere else
- **ScreenScraper:** rejected for now, not permanently — genuinely the strongest match-quality candidate (hash/CRC ROM identification, something none of the title-search-only sources have), but its full quota requires a real per-end-user account EmuBridge cannot provision on the user's behalf, a materially different onboarding cost than an embedded project key; revisit if ROM-hash matching specifically becomes a real, felt need
...
Both fixes are per-window (`Background`/`Foreground` set on the root `Window`/`FluentWindow` element), applied identically across all 5 of EmuBridge's windows.
...
**Consequences:**
- ✅ EmuBridge finally matches what 3 separate docs already claimed — WPF-UI is a real, running dependency, not an aspirational line item
