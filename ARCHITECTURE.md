...
**Status:** Accepted (mechanism); manifest data fully verified for all 15 seed platforms, but only 1 has been interactively install-confirmed — see Consequences
...
All 15 seed platforms now have a `KnownEmulatorCore` entry with real, double-hashed, content-inspected data — the catalog itself is complete. **This is not the same as 15 platforms being interactively proven to install and launch a game.** Only `nes` has actually been through a live "Auto-Install" click that succeeded end-to-end (ADR-14); `snes` and the other 12 are catalog-verified only — see `DEVELOPMENT.md` → Known Limitations for the distinction, restated there so it isn't lost.
...
- ❌ **Only 1 of 15 (`nes`) has been through a real, interactive Auto-Install that succeeded end-to-end.** The other 14 — including `snes`, added and data-verified in the prior update — have never had a live click; the install mechanism (ADR-14) is generic and already proven, so this is a lower-risk gap than "does the mechanism work at all," but it's still an open interactive-confirmation gap, not claimed as closed here.
...
- ❌ The catalog now covers all 15 seed platforms (ADR-11's 2026-08-04 update), but only `nes` has actually been proven to auto-install and launch a game through a real interactive click — the other 14 are data-verified, not interactively confirmed; that distinction is `PLAN.md` → Timeline's next step, not a mechanism gap.
